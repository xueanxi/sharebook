...
# 导入提示词模板
from sharebook.services.extraction_character.prompt.prompts import (
...
class LLMUtils:
    ...
    def __init__(self, llm_config_path: str = "sharebook/config/llm_config.py", extraction_config_path: str = "sharebook/services/extraction_character/config.yaml"):
        """
        初始化LLM工具
    ...
    def _load_novel_type(self, config_path: str) -> str:
        ...
        try:
            ...
            self.logger.error(f"加载小说类型失败: {e}")
            # 返回默认小说类型
            return "玄幻修真"
...
"""
        ...
        try:
            ...
            self.logger.info(f"角色分析完成: {character_name}")
...
"""
        ...
        try:
            ...
            # 记录角色更新日志
            if merged_info != existing_info:
                self.change_logger.log_character_update(
                    original_character=existing_info,
                    updated_character=merged_info,
                    update_reason="LLM智能合并角色信息"
                )
            
            self.logger.info(f"角色信息合并完成: {existing_info.get('姓名', '')}")
            return merged_info
        except Exception as e:
            self.logger.error(f"角色信息合并失败: {e}")
            # 返回简单合并结果
            return {
                "姓名": existing_info.get("姓名", new_info.get("姓名", "")),
                "性别": new_info.get("性别", existing_info.get("性别", "未知")),
