...
```json
{
  ...
  "segments": [
    {
      "segment_id": "e8d1344f-e17d-413d-9cfd-70a7c9b6b998",
      "segment_index": 0,
      "text": "故事文本内容...",
      "metadata": {
        "dialogue_count": 1,
...
为了实现与LLM的解耦，提示词模板被设计为独立的配置文件，位于`src/services/storyboard_to_prompt/config/prompts.py`。

#### 6.2.1 提示词模板结构

提示词配置文件包含以下主要模板：

1. **SINGLE_SCENE_TO_PROMPT_CONVERTER**
   - 单场景到提示词转换模板
   - 处理单个场景数据并生成视觉提示词
   - 包含角色一致性、场景描述和旁白文案生成
