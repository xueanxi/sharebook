...
=======
>>>>>>> b25895dc83d44f68925f5fee0c043b42c242d272
## 1. 功能概述
...
class NovelToComicProcessor:
    def __init__(self, config):
        # 初始化组件
        self.segmenter = IntelligentSegmenter(config.segmentation)
        self.scene_splitter = SceneSplitterAgent(config.llm)
        self.visual_narrative = VisualNarrativeAgent(config.llm)
    ...
    def process_chapter(self, chapter_file, chapter_title, novel_type):
        ...
        all_scenes = []
        for segment in segments:
            scenes = self.scene_splitter.split_segment_to_scenes(segment)
            all_scenes.extend(scenes)
        
        # 4. 生成视觉文案
        storyboards = []
        for scene in all_scenes:
            storyboard = self.visual_narrative.generate_visual_narrative(scene)
            storyboards.append(storyboard)
        
        # 5. 保存结果
        result = self._create_chapter_result(chapter_title, segments, all_scenes, storyboards)
        output_path = self._save_result(result, chapter_file)
...
```python
# 在config/text_splitting_config.py中定义
...
if result.success:
    ...
    print(f"处理失败，错误信息：{result.errors}")
```
